<template>
  <!--     <header class="border-b border-gray-200 bg-gray-100 w-full">
      <div class="container mx-auto">
        <nav class="p-4 flex items-center justify-between">
            <div class="text-lg font-medium hover:scale-110">
                            <div v-if="!isAdmin">
              <Link :href="route('DashboardUser')">Dashboard</Link>
            </div>
            <div v-else>
              <Link :href="route('Dashboard')">Users</Link>
            </div>
          </div>
          <div v-if="!isAdmin">
            <div class="text-xl text-indigo-600 font-bold text-center hover:scale-110">
              <Link :href="route('formation.index')">E-learning</Link>
            </div>
          </div>
          <div v-else class="text-lg font-medium hover:scale-110">
          ...
          <div>
            <div v-if="user" class="flex items-center gap-4">
              ...
              <div v-if="isAdmin" class="btn-primary">
                <Link :href="route('formation.create')" as="button">+ New formation</Link>
              </div>
              <div>
            ...
            <div v-else>
              <div class="flex items-center gap-3">
                <Link :href="route('user-account.create')" as="button">Register</Link>
  ...
  <header class="bg-white shadow-md">
    <nav class="flex justify-between items-center w-[92%] mx-auto">
      <div>
        <Link :href="route('LandingPage')">        <h3 class="font-Zeitung my-4 text-xl text-indigo-700">
          e-Learning
        </h3>
        </Link>
      ...
      <div class="flex items-center gap-6" v-if="!user">
        ...
        <button class="inline-flex w-full justify-center rounded-md bg-indigo-400 px-4 py-2 text-sm font-medium text-white hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
          Sign in
        </button>
        </Link>
      </div>
